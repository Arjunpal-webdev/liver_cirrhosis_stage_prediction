...
RULE: Every st.html() call uses a COMPLETE, self-contained HTML block.
      No HTML is ever split across multiple st.html() calls.
      Uses st.html() (Streamlit 1.31+) instead of st.markdown(unsafe_allow_html=True).
"""
...
">
    ...
    <p style="
        color:#94a3b8; margin:0.5rem 0 1rem 0;
        font-size:0.95rem; font-weight:400; max-width:600px;
    ...
    <div style="display:flex; gap:0.6rem; flex-wrap:wrap;">
        ...
        <span style="background:rgba(168,85,247,0.15);color:#c084fc;
...
# ─────────────────────────────────────────────────────────────────────────────
# Metric Card (returns HTML string — caller does st.markdown(...))
# ─────────────────────────────────────────────────────────────────────────────
def render_metric_card(title: str, value: str, subtitle: str = "",
                       color: str = "#3b82f6", icon: str = "📊") -> str:
    """Return a complete HTML metric card string."""
    subtitle_html = (
        f'<div style="color:#64748b;font-size:0.8rem;margin-top:0.4rem;">{subtitle}</div>'
        if subtitle else ""
    )
    return f"""
    <div style="
...
">
    ...
    <h2 style="font-size:2.8rem;font-weight:900;color:{color};
               margin:0 0 0.25rem 0;letter-spacing:-0.04em;">{stage}</h2>
...
def render_section_header(title: str, subtitle: str = "", icon: str = "📌"):
    ...
    subtitle_html = (
        f'<p style="color:#64748b;margin:0.3rem 0 0 2.25rem;font-size:0.88rem;">{subtitle}</p>'
        if subtitle else ""
    )
    st.html(f"""
<div style="margin:1.5rem 0 1rem 0;">
    <div style="display:flex;align-items:center;gap:0.75rem;">
        ...
        <h3 style="margin:0;font-size:1.3rem;font-weight:700;
                   color:#f1f5f9;letter-spacing:-0.02em;">{title}</h3>
    </div>
    {subtitle_html}
</div>
""")
...
<div style="text-align:center;padding:1rem 0 1.5rem 0;">
    ...
    <div style="font-size:1rem;font-weight:700;color:#f1f5f9;
                letter-spacing:-0.01em;">Liver Cirrhosis AI</div>
...
""")
        ...
        # ── Model Info Cards (ONE self-contained block) ───────────────────
        model_cards = [
...
<div style="
    ...
    ⚠️ For research use only.<br>Not a substitute for medical advice.
</div>
""")
...
# ─────────────────────────────────────────────────────────────────────────────
# Feature Badge (returns HTML string)
# ─────────────────────────────────────────────────────────────────────────────
def render_feature_badge(feature: str, value: str, shap_val: float) -> str:
    """Return HTML for a single feature badge."""
    ...
    <div style="background:{bg};border:1px solid {border};border-radius:10px;
                padding:0.5rem 0.8rem;display:flex;flex-direction:column;gap:0.15rem;">
...
">
    ...
    <p style="color:#64748b;font-size:0.88rem;margin:0;
              max-width:400px;margin-left:auto;margin-right:auto;">{message}</p>
</div>
""")
